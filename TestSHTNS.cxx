...
int main(){
	...
	Qlm[pp] = 0.0;
    Qlm[ LM(shtns, 0, 0) ] = std::complex<double_t>(4,0);
    Qlm[ LM(shtns, 1, 0) ] = std::complex<double_t>(3,0);
	...
	//auto Y11 = Plm(1,1)*std::complex<double_t>(std::cos(phi),std::sin(phi));
	//auto f11Y11 = Y11*std::complex<double_t>(2,3);
	//finput(i) = std::real( f11Y11 + std::conj( f11Y11 ) );
    ...
    size_t index = 0;
    for( auto it = 0; it < shtns->nlat; ++it){
	for( auto ip = 0; ip < shtns->nphi; ++ip){
	    auto phi = ip*M_PI*2/nphi;
	    Q0.col(index) << shtns->st[it]*std::cos(phi),
	...
	//auto Y11 = Plm(1,1)*std::complex<double_t>(std::cos(phi),std::sin(phi));
	//auto f11Y11 = Y11*std::complex<double_t>(2,3);
	//auto fq_dir2 =  std::real( f11Y11 + std::conj( f11Y11 ) );
	interp(i) = SH_to_point(shtns, Qlm, z, phi);
	std::cout<< fquad(i) << " " << interp(i) << std::endl;
    }

    Eigen::VectorXd fq2(numQ);
    fq2 = fquad;

    // Now time for spherical harmonic analysis -- find the coefficients flm
    spat_to_SH(shtns, f, Slm);
    SH_to_spat(shtns, Slm, f);

    std::cout<< "Printing the reverse calculated quad point values"
	<< std::endl;
    for( auto i = 0; i < numQ; ++i)
	std::cout << (fquad(i) - fq2(i)) << " " << (fquad(i) - interp(i))
	    << std::endl;
